...
<html lang="en">
  ...
  <body>
    ...
    <center>
      <form action='../../returning_apis/AddCOPOMapping.php' id='insert_form' method=POST>
        ...
        <table id="customers">
          ...
          <?php
            ...
            echo "<input type='hidden' id='co_list' name='co_list' value='".json_encode($co_list)."' />";
            ...
            for ($k=0; $k<count($co_list); $k++) {
              echo "<tr>";
                ...
                echo "</td>";
                echo "<td>";
                  echo "<input type=text name='$co_list[$k]_co_statement' placeholder='Enter Here' />";
                echo "</td>";
                echo "<td>";
                  echo "<input type=text size=3 name='$co_list[$k]_hours' id='$co_list[$k]_hours' placeholder='hours' />";
                echo "</td>";
                  for ($i=0; $i<$no_pos; $i++) {
                    echo "<td>";
                      echo "<select name='$co_list[$k]_pos[]'>";
  ...
  <script>
    $(document).ready(function(){
      $("#insert_form").submit(function(event){
        var co_list = JSON.parse(document.getElementById("co_list").value);
        var count = 0;
        while (count < co_list.length){
          var sum = 0;
          var hours = document.getElementById(co_list[count]+"_hours").value;
          $("select[name='"+co_list[count]+"_pos[]']").each(function () {
          ...
          if (hours !== "") {
            if (parseInt(hours) === sum) {
              ...
              alert("Please select number of hours equal to total number of hours in "+co_list[count]+"!!");
              event.preventDefault();
              break;
            }
          } else {
            alert("Please enter data in all the fields and then try to submit!!");
            event.preventDefault();
            break;
          }
          count = count + 1;
