    ...
    <center>
      <div class=box>
        <div style=border:solid 5px rgb(0, 0, 0); padding:10px; margin-left: auto;margin-right: auto;>
          <form action='./ViewLOPOMapping.php' method=POST id='viewSatLOPOMapping' ><br>
            <h2>Select Subject</h2><br>
            <select name=batch id=batch><br>
              <option value=0>Select Subject</option>
              <?php
                $user_id = $_SESSION['uid'];
                $query = "SELECT * FROM batch WHERE batch_id IN (SELECT batch_id FROM sat_lo_mapping) AND created_by=$user_id";
...
<script type='text/javascript'>
  $(document).ready(function(){
    $('#viewSatLOPOMapping').submit(function(e){
