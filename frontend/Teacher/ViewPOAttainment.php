    ...
    <center>
      <div class=box>
        <div style=border:solid 5px rgb(0, 0, 0); padding:10px; margin-left: auto;margin-right: auto;>
          <form action='./POAttainment.php' method=POST id='ViewPOAttainment'><br>
            <h2>Select Subject</h2><br>
            <select name=batch id=batch><br>
              <option value=0>Select Subject</option>
              <?php
                $user_id = $_SESSION['uid'];
                $query = "SELECT * FROM batch WHERE batch_id IN (SELECT batch_id FROM co_attainment) AND batch_id IN (SELECT batch_id FROM co_mapping) AND created_by=$user_id";
...
<script>
  $(document).ready(function(){
    $('#ViewPOAttainment').submit(function(e){
