<?php
  ...
  if (isset($_SESSION['uname'])) {
    ...
    if (in_array($extension, $allowed_extension)) {
      ...
      $query = "INSERT INTO marks(marks, batch_id, created, type) VALUES('$student_array', $batch_id, '$current_date', 8)";
      if ($conn-> query($query)) {
        $marks_id = $conn-> insert_id;
        echo "<script>
          alert('Data Inserted Successfully!!');
          window.location.href='./SaveCourseExitAttainment.php?batch_id=$batch_id&marks_id=$marks_id';
          </script>";
          die();
      } else {
        echo "<script>
          alert('Some error occured, please try again!!');
          window.location.href='../frontend/Teacher/CourseExitMarks.php';
          </script>";
        die();
      }
    } else {
